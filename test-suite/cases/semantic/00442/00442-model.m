...
The model contains one compartment called C.
  ...
  Species S4 is labeled as constant and therefore cannot be changed by rules or reactions.
...
[{width:30em,margin-left:5em}|        | Trigger    | Delay | Assignments |
 | Event1 | $S2 < S4$ | $1$   | $S1 -> 2 \x 10^-15$    |]
 | Event2 | $S3 > 2 \x 10^-15$ | $1.5$   | $S3 -> 1 \x 10^-15$    |]
...
|              Initial amount of S3:| $1.9 \x 10^-15$  | mole                       |
...
addSpecies[ S3, initialAmount -> 1.9 10^-15];
addSpecies[ S4, initialAmount -> 1.0 10^-15, constant -> True];
...
addEvent[ trigger -> S2 < S4, delay->1, eventAssignment -> S1->2 10^-15 ];
addEvent[ trigger -> S3 > 2/(10^15), delay->1.5, eventAssignment -> S3->1 10^-15 ];
