...
synopsis:      Basic two reactions with three species in one compartment
               and two events, subject to delays, that assigns value to a 
...
The model contains one compartment called C.
  There are three species called S1, S2 and S3 and four parameters called k1, k2, k3 and k4.
...
[{width:30em,margin-left:5em}|        | Trigger    | Delay | Assignments |
 | Event1 | $S1 < 0.75$ | $0.25$   | $S2 = add(k3, k4)$    |
 | Event2 | $S3 > 1.2$ | $1.5$   | $S1 = 1 * divide(k3, k4)$    |]
...
addReaction[ S3 -> S1 + S2, reversible -> False,
	     kineticLaw -> k2 * S3 * C ];
addEvent[ trigger -> S1 < 0.75, delay->0.25, eventAssignment -> S2->add[k3, k4] ];
addEvent[ trigger -> S3 > 1.2, delay->1.5, eventAssignment -> S1->1*divide[k3, k4] ];
