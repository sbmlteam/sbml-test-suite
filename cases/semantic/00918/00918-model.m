(* 

category:      Test
synopsis:      Model varying compartment using rules only.
componentTags: Compartment, RateRule, InitialAssignment 
testTags:      NonConstantCompartment, NonUnityCompartment, 2D-Compartment, InitialValueReassigned
testType:      TimeCourse
levels:        2.2, 2.3, 2.4, 3.1
generatedBy:   Analytic

The model contains one varying 2-dimensional compartment called c.

The model contains one rule:

[{width:30em,margin-left:5em}|  *Type*  |  *Variable*  |  *Formula*         |
 | Rate                                 | c           | $1.25 * c$  |]

The model contains one initialAssignment:

[{width:30em,margin-left:5em}| Variable | Formula |
 | c       | $3/2$  |]

Note: SBML's InitialAssignment construct override any declared initial
values.  In this model, the initial value for the compartment c is
inconsistent with the value returned by the InitialAssignment.


The initial conditions are as follows:

[{width:30em,margin-left:5em}| |*Value*         |*Units*  |
|Size of compartment c         |$2$ |metre^2^        |]

Note: The test data for this model was generated from an analytical
solution of the system of equations.


*)

newcase[ "00918" ];

addCompartment[ c, constant -> False, size -> 2, spatialDimensions -> 2 ];
addInitialAssignment[ c, math -> 3.3/2];
addRule[ type->RateRule, variable -> c, math -> 1.25 * c];

makemodel[]
