...
var is_ff3_win = (clientPC.indexOf('firefox/3')!=-1 || clientPC.indexOf('minefield/3')!=-1) && clientPC.indexOf('windows')!=-1;
var is_ff3_x11 = (clientPC.indexOf('firefox/3')!=-1 || clientPC.indexOf('minefield/3')!=-1) && clientPC.indexOf('x11')!=-1;
...
if (typeof stylepath != 'undefined' && typeof skin != 'undefined') {
	if (is_opera_preseven) {
		...
		document.write('<link rel="stylesheet" type="text/css" href="'+stylepath+'/'+skin+'/Firefox2WinFixes.css">');
	} else if (is_ff3_win) {
		document.write('<link rel="stylesheet" type="text/css" href="'+stylepath+'/'+skin+'/Firefox3WinFixes.css">');
	}
}
...
function runOnloadHook() {
	...
	// 2007-11-08 mhucka@caltech.edu.  See below.
	// nukeAccesskeys();
}
...
/**
 * 2007-11-08 mhucka@caltech.edu
 * Remove the key bindings put in place by MediaWiki because
 * I CANNOT STAND IT WHEN A WEB PAGE REBINDS MY KEYBOARD.
 * MediaWiki should make this a user preference setting, not a global 
 * setting forced on every user.  How can I turn it off for myself?
 * I see no way to do it, so I have to use this sledgehammer.
 * Based http://en.wikipedia.org/wiki/Wikipedia:WikiProject_User_scripts/Scripts/removeAccessKeys
 */

function nukeAccesskeys() {
  removeKeys(document.getElementsByTagName('textarea'));
  removeKeys(document.getElementsByTagName('input'));
  removeKeys(document.getElementsByTagName('label'));
  removeKeys(document.getElementsByTagName('a'));
}

function removeKeys(nodeList) {
  for ( var i = 0; i < nodeList.length; i++ ) {
    nodeList[i].removeAttribute("accesskey");
  }
}

hookEvent("load", nukeAccesskeys);
