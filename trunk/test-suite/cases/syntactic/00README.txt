 ...
 Please report problems with the SBML Test Suite using the tracker at
  ...
  | 6. Interpretation of the summary report                       |
   `--------------------------------------------------------------'
...
further input.  And intepreters that accept all SBML models regardless
...
All the folders in this directory are auto-generated by the program
'createSyntacticTests', whose source code can be found in
test-suite/src/utilities/createSyntacticTests.  For more information
on how to compile and run that program, see
test-suite/src/utilities/createSyntacticTests/README.txt
...
Validation rules that appear in SBML core specifications (from any
SBML Level or Version) are simply labeled with the number of the
...
may have mulitple warning messages, however.  Each message in the file
is demarked by a line of dashes, and contains information as in this
...
In this root directory is the file 'summary.csv', which contains
information about every file in the syntactic test suite.  Each line
lists the filename, the validation ID, and other information about
each file, ending with a list of all the error and warning messages
that model produces.
