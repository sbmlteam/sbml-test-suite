...
packagesPresent: 
 ...
 This model is the exactly the same as model 00035, with literally no differences at all.  It seems that the kinetic law was supposed to be slightly different:

<  k1*(100-2*P2)*(100-2*P2-1)/2
---
>  k1*(100-2*P2)*((100-2*P2)-1)/2

 but those two infix lines produce identical MathML.
...
| Dimerisation: -> P2 | $(k1 * (100 - 2 * P2) * (100 - 2 * P2 - 1)) / 2$ |
